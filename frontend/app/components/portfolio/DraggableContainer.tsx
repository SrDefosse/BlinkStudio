    ...
    return (
        <GridVariantContext.Provider value={variant}>
            <div className="h-dvh overflow-hidden w-full">
                <motion.div
                    className="h-dvh overflow-hidden w-full"
                >
                    <motion.div
                        className={cn(
                            "grid h-fit w-fit cursor-grab grid-cols-[repeat(2,1fr)] bg-[#1a1d18] active:cursor-grabbing will-change-transform",
                            className,
                        )}
                        drag
                        dragMomentum={true}
...
export const GridBody = memo(
    ({
        ...
        const gridBodyStyles = cva("grid h-fit w-fit", {
            variants: {
                variant: {
                    default: "grid-cols-[repeat(2,45vw)] gap-4 p-4 md:grid-cols-[repeat(4,22vw)] md:gap-8 md:p-8",
                    masonry: "grid-cols-[repeat(2,45vw)] gap-x-4 px-4 md:grid-cols-[repeat(4,22vw)] md:gap-x-12 md:px-8",
                    polaroid: "grid-cols-[repeat(2,45vw)] gap-x-4 px-4 md:grid-cols-[repeat(4,22vw)] md:gap-x-12 md:px-8",
                },
            },
            defaultVariants: {
                variant: "default",
